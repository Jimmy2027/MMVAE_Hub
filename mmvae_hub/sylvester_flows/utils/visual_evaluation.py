...
def plot_reconstructions(data, recon_mean, loss, loss_type, epoch, args):
    ...
    if epoch == 1:
        ...
        plot_images(args, data.data.cpu().numpy()[0:9], args.snap_dir + 'reconstruction/', 'real',
                    size_x=3, size_y=3)
    ...
    plot_images(args, recon_mean.data.cpu().numpy()[0:9], args.snap_dir + 'reconstruction/', fname,
                size_x=3, size_y=3)
